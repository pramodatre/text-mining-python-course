...
class DocStatsScikit:
    def __init__(self, docs: dict) -> None:
        self.docs = docs
        self.tf_idf_model = TfidfVectorizer(tokenizer=preprocess_text)
        abstracts = self.get_all_abstracts_in_a_list()
        self.tf_idf_vectors = self.tf_idf_model.fit_transform(abstracts)
        print(self.tf_idf_vectors)
        self.document_vector_map = self.get_document_vector_map()
        print(self.document_vector_map)
        self.vocabulary = self.get_vocabulary()

    def get_unit_vector_for_text(self, text):
        return self.tf_idf_model.transform([text]).toarray()[0]
    ...
    def get_document_vector_map(self):
        return {
            doc_id: self.tf_idf_model.transform([self.docs[doc_id]]).toarray()[0]
            for doc_id in self.docs
        }

    def get_vector(self, text):
        return self.tf_idf_model.transform(text).toarray()

    def get_vocabulary(self) -> list:
        return self.tf_idf_model.get_feature_names_out()

    def get_all_abstracts_in_a_list(self) -> list:
        return [self.docs[doc_id] for doc_id in self.docs]
